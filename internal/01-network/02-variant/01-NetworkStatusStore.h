...
class NetworkStatusStore : public INetworkStatusStore {
    ...
    Public Void SetState(Bool networkConnected, ULong networkConnectionId) override {
        ...
        public: static INetworkStatusStorePtr GetInstance() {
            static INetworkStatusStorePtr instance(new NetworkStatusStore());
            return instance;
        }
};

template <>
struct Implementation<INetworkStatusStore> {
    using type = NetworkStatusStore;
};
